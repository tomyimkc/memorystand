...
def _describe_topology(conn) -> str:
    """Describe the cluster this run actually hit, rather than assuming one.

    Returns a phrase for the report header. Asking the cluster how many nodes it has is
    the whole point: the previous template asserted "one local single-node container" in
    every report, so the 3-node run and the CockroachDB Cloud run each published a
    caveat denying what their own headline said.

    Node count comes from `SHOW node_id` plus a gossip count where available; if neither
    can be read, this says so instead of guessing, because "unknown" is a true statement
    and "single-node" was not.
    """
    host = ""
    try:
        dsn = db.dsn()
    ...
    managed = "cockroachlabs.cloud" in host
    nodes = None
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT count(DISTINCT node_id) FROM crdb_internal.gossip_nodes")
            nodes = cur.fetchone()[0]
        conn.commit()
    except Exception:  # noqa: BLE001 - restricted on Cloud Basic and in v26.2
        conn.rollback()

    if managed:
        where = "a **managed CockroachDB Cloud cluster**"
    elif host.startswith(("localhost", "127.0.0.1")):
        where = "a **local CockroachDB container**"
    else:
        where = f"the CockroachDB cluster at `{host or 'an unspecified host'}`"

    if nodes:
        return f"{where} ({nodes} node{'s' if nodes != 1 else ''} visible)"
    return f"{where} (node count not readable from this connection)"
...
def write_report(
    ...
    # Describe the topology this run ACTUALLY used, rather than asserting one.
    # The previous version hardcoded "single-node local container ... not CockroachDB
    # Cloud, not multi-node" into every report -- including the 3-node 250k run and the
    # CockroachDB Cloud run, each of which then contradicted its own headline two
    # paragraphs down. A generated caveat that can be false is worse than no caveat: it
    # reads as careful while being wrong.
    topology = _describe_topology(conn)

    body = f"""# MemoryStand -- retrieval load test results

Generated by `scripts/loadtest.py`. This is a **single-run measurement** against
{topology} (see "Environment" below), not a controlled, repeated,
statistically-powered benchmark. Treat the numbers as directional evidence that the
vector index changes the query plan and materially affects recall latency at this row
count -- not as an SLA.
...
- `{INDEXED_TABLE}`: the real, already-deployed table from `db/schema.sql`,
  ...
  vector_cosine_ops)`. The `verdict` column is part of the prefix and matters: drop it
  and the plan below stops using the index.
...
- Single run, not repeated across seeds. Topology as detected: {topology}.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        write_report(
            conn, report_path, args, version, command,
