...
a **local CockroachDB container** (node count not readable from this connection) (see "Environment" below), not a controlled, repeated,
...
- Rows seeded this run: 10000 per table, tagged
  ...
  20101 (may include rows from other concurrent work on this
...
|---|---:|---:|---:|
| agent_memories (vector-indexed) | 10000 | 3.81 | 2623.4 |
| agent_memories_noindex (no index) | 10000 | 1.48 | 6750.8 |

## Recall latency (k=5, 200 queries/table, identical query set against both tables)

| Table | p50 (ms) | p95 (ms) | p99 (ms) | mean (ms) | samples |
|---|---:|---:|---:|---:|---:|
| agent_memories (vector-indexed) | 2.38 | 2.76 | 2.91 | 2.39 | 200 |
| agent_memories_noindex (no index) | 23.61 | 28.95 | 35.36 | 24.41 | 200 |

Indexed table is **9.92x faster at p50** and **12.14x
...
└── • render
    ...
    └── • lookup join
        ...
        └── • vector search
              ...
              target count: 5
              prefix spans: [/'e5dc63fc-a05c-4231-b97a-5ecb0fee1a25'/'accepted' - /'e5dc63fc-a05c-4231-b97a-5ecb0fee1a25'/'accepted']
```
...
- Single run, not repeated across seeds. Topology as detected: a **local CockroachDB container** (node count not readable from this connection).
