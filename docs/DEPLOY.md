...
| **Amazon Bedrock** (Claude Converse + Titan embeddings) | No free tier; billed per token/request | The only genuinely metered cost here, and it is small: `/decide` calls Bedrock only when the caller does *not* supply `action` itself (this dashboard always supplies it, so it skips Bedrock entirely -- see `frontend/app.js`'s API-contract comment), and `/ingest` embeds one short string per call. A judging window's worth of manual clicking is cents, not dollars. |
